...
Organization icons are original letter-tile icons generated by
``scripts/generate_images.py`` (NOT the organizations' trademarked logos —
see that script for details). To use an official logo you have rights to,
replace the matching PNG in ``assets/icons/``.
"""
...
def icon_entry(icon_name: str, heading: str, body: str) -> None:
    """Render one education/career entry with its organization icon.

    Args:
        icon_name: Filename stem of the icon in ``assets/icons`` (e.g.
            ``"umich"`` for ``assets/icons/umich.png``).
...
audience of more than 26,000 followers.
    ...
    st.caption(
        "Organization icons are original letter-tiles created for this app — "
        "not official logos, which are trademarked."
    )
